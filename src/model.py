...
class ZindiBaseModel(object):
    ...
    def define_loss(self):
        ...
        self.train_loss = tf.multiply(masked_loss, train_loss_mask, name='train_loss')

        # val loss
        val_loss_mask = tf.reduce_all(tf.math.equal(self.field_id, self.field_mask), -1)
        val_loss_mask = tf.cast(val_loss_mask, tf.float32)
        self.val_loss = tf.multiply(masked_loss, val_loss_mask, name='val_loss')

        tf.summary.scalar('train_loss', tf.reduce_sum(self.train_loss))
        tf.summary.scalar('val_loss', tf.reduce_sum(self.val_loss))

    def define_optimizer(self, learning_rate, session):
...
class VggFcnBaseModel(ZindiBaseModel):
    ...
    def build_segmentation_head(
        ...
        W8 = weight_variable([1, 1, 4096, self.nb_class], name="W8")
        b8 = bias_variable([self.nb_class], name="b8")
        ...
        W_t1 = weight_variable([2, 2, deconv_shape1[3].value, self.nb_class], name="W_t1")
        ...
        W_t2 = weight_variable([2, 2, deconv_shape2[3].value, deconv_shape1[3].value], name="W_t2")
        ...
        W_t3 = weight_variable([8, 8, self.nb_class, deconv_shape2[3].value], name="W_t3")
        b_t3 = bias_variable([self.nb_class], name="b_t3")
